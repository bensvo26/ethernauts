...
Because the call is routed through the contract, `tx.origin` (your EOA) and `msg.sender` (the attack contract) are different, passing the condition and making you the owner.

## Takeaway
Never use `tx.origin` for authorization. It breaks the moment a call passes through another contract and enables certain attacks. Use `msg.sender` to authenticate the immediate caller.