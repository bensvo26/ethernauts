...
## Exploit
Two console commands were used. First get the function selector for `pwn()`, then send it as raw calldata so the `fallback` fires and delegates into `Delegate.pwn()`:
...
## Takeaway
`delegatecall` executes external code against your own storage. Forwarding arbitrary calldata into a `delegatecall` lets a caller run any of the target's functions in your context, which can clobber critical state like `owner`. Never expose an unguarded `delegatecall` to a user controlled input.